* 2022_cross_eu_silc_r_ver_2023_release2_v2.do 
*
...
* Country files are delivered in the format UDB_c*country_stub*22R.csv
* 
* (c) GESIS 2024-07-08
*
* PLEASE NOTE
...
*
* Pforr, Klaus, Johanna Jung and Carl Riemann (2024): 2022_cross_eu_silc_r_ver_2023_release2_v2.do, 1st update.
...
local log_file "$log/eusilc_2022_r" ;
...
local stata_file "$log/eusilc_2022_r_cs" ;
...
foreach CC in AT BE BG /*CH*/ CY CZ DE DK EE EL ES FI FR HR HU IE /*IS*/ IT LT LU LV MT NL /*NO*/ PL PT RO /*RS*/ SE SI SK /*UK*/ { ;
      cd "$csv_path/`CC'/2022" ;
	  import delimited using "UDB_c`CC'22R.csv", case(upper) asdouble clear stringcols(2 28 32);
...
label variable RB285 "Duration of stay in the country of residence in completed years (SI: missing)" ;
...
label variable RL080 "Remote education (Optional)" ;
label variable RL080_F "Flag" ;
...
-7 "Not applicable (RB010 < 2021)"
;
label define RB082_VALUE_LABELS
80 "SI: 80 and above"
;
...
1 "Filled"      
-1 "Missing" 
-2 "Not applicable (Have no spouse/partner or spouse/partner is not a household member)"
; 
...
-1 "Missing"
-2 "Not applicable (person at pre-school because of age, at compuls school, aged 12+); (RL010>0 and RL020>0) not possible)"
;
...
4 "Not possible to establish a source"
-1 "Missing"
-2 "Not applicable (person not at compuls sch bec of age, at compuls school, aged 12+); (RL010>0 and RL020>0) not possible)"
;
...
;
label define RL080_F_VALUE_LABELS
 1 "Collected via survey/interview"
 2 "Collected from administrative data"
 3 "Imputed"
 4 "Not possible to establish source"
-1 "Missing"
-2 "Not applicable (person is more than 12 years old)"
-7 "Not applicable (RB010 not equal 2022)"
...
0 "BG: not defined in data documentation"
1 "Collected via survey/interview"
2 "Collected from administrative data"
3 "Imputed"
4 "Not possible to establish a main source"
-1 "Missing"
-2 "Not applicable (one person household/relationship with themselfe in the matrix)"
...
label values RB082 RB082_VALUE_LABELS ;
...
label values RL080_F RL080_F_VALUE_LABELS ;
...
label data "Personal register file 2022" ;
